...
c.......................................................................
c     calling sequence: matgen > gasmomrh > advecti
c***********************************************************************
c
      include'titan.imp'
      include'titan.par'
...
c-----------------------------------------------------------------------
c
      f0 (xk,xp) =             ( (xk+xp)/2.D0 )**( 1.5D0*xmu )
      f0r(xk,xp) = .75D0*xmu * ( (xk+xp)/2.D0 )**( 1.5D0*xmu - 1.0D0 )
c
c-----------------------------------------------------------------------
...
c
      ep1(iu, jr, k) = ep1(iu, jr, k)    - ( avchi(k) * fr(k) / cc )
     .        * 0.5D0 * thet * (+ d(k  ) * rmu(k+1) ) * rn(k+1)
c
      e00(iu, jr, k) = e00(iu, jr, k)    - ( avchi(k) * fr(k) / cc )
     .        * 0.5D0 * thet * (- d(k  ) * rmu(k  ) 
     .                          + d(k-1) * rmu(k  ) ) * rn(k  )
c
      em1(iu, jr, k) = em1(iu, jr, k)    - ( avchi(k) * fr(k) / cc )
     .        * 0.5D0 * thet * (- d(k-1) * rmu(k-1) ) * rn(k-1)
c
c     equations rm48 & rm49
c
      e00(iu, jd, k) = e00(iu, jd, k)    - avchi(k) * ( fr(k) / cc )
     .        * 0.5D0 * thet * dn(k  ) * dvol(k  ) 
     .        - ( avchi(k) / chif (k  ) )**2 * thet * ( fr(k) / cc )
     .        * 0.5D0 *      chifn(k  ) * dlcfdldn(k  ) 
     .        * 0.5D0 * ( d(k-1) * dvol(k-1) + d(k) * dvol(k) )
c
      em1(iu, jd, k) = em1(iu, jd, k)    - avchi(k) * ( fr(k) / cc )
     .        * 0.5D0 * thet * dn(k-1) * dvol(k-1)
...
c-----------------------------------------------------------------------
c
      if (leobc .lt. 3 .or. llobc .eq. 1) then
            rhs(iu,     k) = 0.0D0
            e00(iu, ju, k) = 1.0D0
      end if
c
...
c
c.......................................................................
c           radiation force; equations bc93 - bc97
c.......................................................................
c
            e00(iu, jr, k) = e00(iu, jr, k) 
     .                                     - ( chif (k-1) * fr(k) / cc )
...
c
     ...
     .              * thet * (- d (k-1) * rmu(k-1) ) * rn(k-1) 
c
            em1(iu, jd, k) = em1(if, jd, k) 
     .              - thet * dlcfdldn(k-1) * ( chifn(k-1) * fr(k) / cc )
     .                        * d (k-1) * dvol(k-1) 
     .               -                       ( chif (k-1) * fr(k) / cc )
     .                 * thet * dn(k-1) * dvol(k-1) 
c
            em1(iu, jt, k) = em1(if, jt, k) 
     .              - thet * dlcfdltn(k-1) * ( chifn(k-1) * fr(k) / cc )
     .                       * dn(k-1) * dvol(k-1) 
c
            e00(iu, jf, k) = e00(iu, jf, k) 
     .                               - thet * chif (k-1) * frnom(k) / cc
     .                            * d (k-1) * dvol (k-1) 
c
c.......................................................................
